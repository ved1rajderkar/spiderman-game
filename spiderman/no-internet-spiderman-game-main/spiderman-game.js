...
function SpidermanGame(opts) {
	...
	this.baseWidth = 711;  // Original canvas width
    this.baseHeight = 400; // Original canvas height
    this.scale = 1;       // Current scale factor
}
...
SpidermanGame.prototype.showPauseMenu = function() {
    ...
    this.updateMenuPosition(this.pauseMenu);
};
...
SpidermanGame.prototype.showGameoverMenu = function() {
    ...
    this.updateMenuPosition(this.gameoverMenu);
};

// Add new helper method to handle menu positioning
SpidermanGame.prototype.updateMenuPosition = function(menu) {
    if (!menu) return;
    
    // Menu will be centered automatically by CSS transform
    menu.style.left = "50%";
    menu.style.top = "50%";
};

SpidermanGame.prototype.pause = function() {
    if (this.gameIsOver) return; // Don't pause if game is over
    this.paused = true;
    this.showPauseMenu();
};

SpidermanGame.prototype.unpause = function() {
    if (this.gameIsOver) return; // Don't unpause if game is over
    this.paused = false;
...
SpidermanGame.prototype.toggleFullscreen = function() {
...
SpidermanGame.prototype.handleFullscreenChange = function() {
    if (document.fullscreenElement) {
        ...
        // Set canvas CSS dimensions for display scaling
        this.canvas.style.width = `${newWidth}px`;
        this.canvas.style.height = `${newHeight}px`;
        
        // Don't change the scale - keep internal dimensions constant
        this.scale = 1;
    } else {
...
SpidermanGame.prototype.initMobileControls = function() {
    const self = this;
    const buttons = document.querySelectorAll('.d-pad-btn, .action-btn');
    ...
    if (leftBtn) {
        leftBtn.addEventListener('touchstart', (e) => {
            e.preventDefault();
            self.spiderman.keydown(KEY.ARROW_LEFT);
        });
        leftBtn.addEventListener('touchend', (e) => {
            e.preventDefault();
            self.spiderman.keyup(KEY.ARROW_LEFT);
        });
    }
    ...
    if (rightBtn) {
        rightBtn.addEventListener('touchstart', (e) => {
            e.preventDefault();
            self.spiderman.keydown(KEY.ARROW_RIGHT);
        });
        rightBtn.addEventListener('touchend', (e) => {
            e.preventDefault();
            self.spiderman.keyup(KEY.ARROW_RIGHT);
        });
    }
    ...
    if (shootBtn) {
        shootBtn.addEventListener('touchstart', (e) => {
            e.preventDefault();
            self.spiderman.keydown(KEY.SPACEBAR);
        });
        shootBtn.addEventListener('touchend', (e) => {
            e.preventDefault();
            self.spiderman.keyup(KEY.SPACEBAR);
        });
    }
};
...
Enemy.prototype.shoot = function() {
	...
	projectile.y = this.y + (this.stateImg.height * this.scale / 2) - (knife.height * self.scale / 4);
